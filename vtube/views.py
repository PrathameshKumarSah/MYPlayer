...
def home(request):
	...
	params = {'auth':checkUser(request)}
	return render(request, "index.html", params)

def checkUser(request):
	if request.user.is_anonymous:
		auth = {'user':request.user, 'isChannel':False}
	else:
		channel = Channels.objects.filter(user=request.user)
		if len(channel) != 0:
			basic = {'channelId':channel[0].id}
			print(channel[0].channelPic)
			auth = {'user':request.user, 'isChannel':True,'channelId':channel[0].id}
		else:
			auth = {'user':request.user, 'isChannel':False}
	return auth;
...
def getTrending(request):
	...
	params = {'video':trendingv, 'auth': checkUser(request)}
	print(params)
...
def handleChannel(request, id):
	...
	if len(Channels.objects.filter(id=id)) > 0:
		...
		basic = {'channelId':channel.id,'title':channel.channelTitle, 'about':channel.about,  'channelPic':channel.channelPic}
		# print(basic)
		if request.user.is_anonymous:
			user_videos = Videos.objects.filter(channel=channel)
			params = {'authentic':False, 'basic':basic, 'isChannel':False, 'videos':user_videos, 'auth': checkUser(request)}
			...
			params = {'authentic':True, 'basic':basic, 'isChannel':isUserChannel(request.user), 'videos':user_videos, 'auth': checkUser(request)}
			return render(request, 'channel.html', params)
		else:
			user_videos = Videos.objects.filter(channel=channel)
			params = {'authentic':False, 'basic':basic, 'isChannel':isUserChannel(request.user), 'videos':user_videos, 'auth': checkUser(request)}
			return render(request, 'channel.html', params)
	else:
		return HttpResponse("<h1>404 - Not Found</h1>")
...
def editVideo(request, id):
	if request.method == 'POST':
		...
		video = Videos.objects.get(id=id)
		params = {'video':video, 'auth': checkUser(request)}
...
def watchVideo(request,id):
	...
	if not request.user.is_anonymous:
		...
		history.save()
	return render(request, 'watchVideo.html',{'video':Videos.objects.get(id=id), 'auth': checkUser(request)})


def yourVideo(request):
	if request.user.is_anonymous:
		params = {'videos':[], 'auth': checkUser(request)}
	else:
		videos = list(Videos.objects.filter(user=request.user))
		# print(videos)
		params = {'videos':videos, 'auth': checkUser(request)}
...
def userHistoryFunc(request):
	if request.user.is_anonymous:
		...
		params = {'video':[], 'auth': checkUser(request)}
		...
		if (len(uhistory)>0):
			...
			# print(history_list)
		params = {'video':history_list, 'auth': checkUser(request)}
		return render(request, "userHistory.html", params)

		# else:
		# 	return render(request, "userHistory.html", params = {'video':[], 'auth': checkUser(request)})
